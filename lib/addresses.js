...
AddressController.prototype.transformUtxo = function(utxoArg) {
  ...
  if (utxoArg.height && utxoArg.height > 0) {
    ...
    utxo.confirmations = this.node.services.fantasygoldd.height - utxoArg.height + 1;
  } else {
...
AddressController.prototype.transformAddressHistoryForMultiTxs = function(txinfos, options, callback) {
  ...
  return async.eachSeries(items, function(txItem, callback) {
    return self.transactionService.addReceiptIfTransfersExists(txItem, function(err, tx) {
      return callback(err);
    });
  }, function(err) {

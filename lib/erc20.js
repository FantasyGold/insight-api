...
Erc20Controller.prototype.findFGC20Contracts = function(req, res) {
...
Erc20Controller.prototype.getTransfers = function(req, res) {
    ...
    var self = this,
        contractAddress = req.params.contractAddress,
        withReceipt = req.query.withReceipt,
        offset = req.query.offset,
        limit = req.query.limit,
        addresses = req.query.addresses,
    ...
    try {
        withReceipt = withReceipt ? JSON.parse(withReceipt) : false;
    } catch (err) {
        withReceipt = true;
    }
    ...
    return async.waterfall([function(callback) {
        ...
        return async.eachOfSeries(dataFlow.transfers, function(transfer, index, callback) {

            return self.node.getTransactionReceipt(transfer.tx_hash, function(err, response) {

                dataFlow.transfers[index] = dataFlow.transfers[index].toJSON();
                ...
                dataFlow.transfers[index].receipt = response;

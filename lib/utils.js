...
var MIN_ESTIMATE_SMART_FEE_PER_KB = 0.00001;
...
UtilsController.prototype.estimateSmartFee = function(req, res) {
  ...
  async.map(nbBlocks, function(n, next) {
    ...
    self.node.services.fantasygoldd.estimateSmartFee(num, function(err, fee) {
      if (err) {
        return next(err);
      }
      next(null, [num, fee]);
...
UtilsController.prototype.minEstimateSmartFee = function(req, res) {
    var self = this;
    var nBlocks = req.query.nBlocks || '6';

    return self.node.services.fantasygoldd.estimateSmartFee(nBlocks, function(err, fee) {
        ...
        return res.jsonp({fee_per_kb: MIN_ESTIMATE_SMART_FEE_PER_KB});

...
AddressBlocksMinedService.prototype.processBlock = function (blockHeight, next) {
    ...
    return async.waterfall([function (callback) {
        return self.node.services.fantasygoldd.getJsonBlock(blockHeight, function (err, response) {
        ...
        switch (block.flags) {
            case fantasygoldcore.Block.PROOF_OF_STAKE:
                txHash = block.tx[1];
                break;

            case fantasygoldcore.Block.PROOF_OF_WORK:
                txHash = block.tx[0];
                break;
        }
